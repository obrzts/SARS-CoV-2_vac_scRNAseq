---
title: "Analysis of transcriptome and Ig repertoire from B cels responding to SARS-CoV-2 vaccination"
...
Reads were mapped to human genome (GRCh38-2020-A) using 10x Genomics Cell Ranger v6.1.2 multi. Cell filtering and read count normalization was done using preprocess_transcriptome.R. Full-length V(D)J contigs assembled with Cell Ranger were aligned to the IMGT reference using IgBLAST v1.20.0. Filtering Ig repertoire data and clone definition was done using preprocess_repertoire.R.
...
## Figure 1D
UMAP projection of single cell transcriptional profiles colored by population.
...
```{r, fig.width = 4.9, fig.height = 2.9, dpi=300}
...
## Figure 1F-H
IGHV+IGK/LV somatic hypermutation (SHM) counts across the cell populations, class-switched antibody frequency, and clone size distribution.

```{r, fig.width = 7, fig.height = 2.5, dpi=300}
p1 <- cell_metadata %>%
  filter(heavy_shm < 80) %>%
...
## Figure K-L
Frequency of antibody secreting and B cell populations in S+ cells over time.

```{r, fig.width = 7, fig.height = 2.2, dpi=300}
...
## Figure S1G
Frequency of persistent clones. Dots show average and error bars SEM across donors.
...
## Figure S1H
Frequency of TFRC+ among MBCs and naïve B cells. TFRC+ cell were defined by presence of at least one TFRC transcript. Dotted vertical lines indicate prime and boost.


# Figure 2: Low-SHM but not high-SHM S+ IgG MBCs display signs of ongoing GC reaction

## Figure 2A
IGHV + IGK/LV SHM counts in S+ IgG MBCs. Dotted vertical lines indicate prime and boost.
...
```{r, fig.width=2.5, fig.height=3, dpi=300}
...
## Figure 2F-H
UMAP projection of high- and low-SHM S+ IgG MBCs colored by SHM level or cluster defined by unsupervised transcriptome clustering. Frequency of high- and low-SHM cells in the transcriptome clusters.
...
```{r, fig.width = 7, fig.height = 4.2, dpi=300}
...
## Figure 2I
Genes differentially expressed between low- and high-SHM associated clusters
...
## Figure 2J
Signalling pathways and transcriptional programs enriched in genes differentially expressed between low- and high-SHM associated clusters.

Signaling pathways

```{r, fig.width = 5, ig.height = 3, dpi=300}
...
```{r, fig.width = 4.5, fig.height = 2.5, dpi=300}
...
## Figure S2A
SHM count distribution in IGHM and IGHG expressing S+ MBCs at the indicated time points. Dashed lines show thresholds separating the first density peak from the rest of the distribution.

```{r, fig.width=14, fig.height=4, dpi=300}
p1 <- ig_data %>%
...
```{r, fig.width = 4, fig.height=2.5, dpi=300}
group_freq %>%
...
```{r, fig.width = 2.5, fig.height =2.7, dpi=300}
ig_data2 %>%
...
## Figure 3B
BCR signaling pathway gene expression in S+ IgG MBCs. (C-F) Ig variable segment gene usage of S+ MBCs.
...
```{r, fig.width = 1.6, fig.height = 2.8, dpi=300}
memory_IgG %>%
  merge(aucs) %>%
  mutate(group = str_replace(group, "-", "-\n")) %>%
...
## Figure 3C
Ig variable segment gene usage of S+ MBCs. Principal component analysis (PCA) on paired chain V gene usage (dots represent MBC populations from individual donors).
...
## Figure 3D
Frequency of individual IGHV genes associated with S-binding (left) or their pooled frequency (right)
...
## Figure 3E
Frequency of individual IGK/LV genes associated with S-binding (left) or their pooled frequency (right).
...
```{r, fig.width = 2, fig.height = 2.7, dpi=300}
...
```{r, fig.width = 1.3, fig.height = 2.5, dpi=300}
x <- ig_data2 %>%
...
Plots for Figure 4 were generated using R as well. Get in touch if you are interested in the code.

# Figure 5: Naïve B cells generate less mature but more potent IgG expressing ASCs compared to pre-existing MBCs

## Figure 5A
Principal component analysis (PCA) on paired chain V gene usage of IgM and IgG cells. Dots represent MBCs or ASCs populations from individual donors.
...
## Figure 5C
Frequency of high- and low-SHM IgG ASCs over time. Dotted vertical lines indicate prime and boost.

```{r, fig.width = 3.3, fig.height=2, dpi=300}
ig_data_pc <- ig_data %>%
...
Diffusion map of ASCs colored by pseudotime (left) or maturation stage (right). PB, plasmablasts (pseudotime ⩽0.33); early PC, early plasma cell (0.33 < pseudotime < 0.66); mature PC, mature plasma cell (pseudotime ⩾0.66).
...
## Figure 5F
Expression of genes contributing in mature plasma cell phenotype along the pseudotime trajectory.
...
```{r, fig.width = 12, fig.height = 3, dpi=300}
line_size = 0.8
pointer_size = 0.3
...
## Figure 5G
Frequency of mature PC among low- and high-SHM IgG ASCs across time points.
...
## Figure S5C-D
Spike barcode read count distribution in high- and low-SHM ASCs; IgM and IgG ASCs are pooled; frequency of S+ cells in subpopulations of ASCs defined by the Spike barcode read count.
...
```{r, fig.width = 6, fig.height = 2.5, dpi=300}
x %>%
...
## Figure S5F
Frequency of early PC and PB among low- and high-SHM IgG ASCs across time points.
...
```{r, fig.width=3, fig.height=3.2, dpi=300}
